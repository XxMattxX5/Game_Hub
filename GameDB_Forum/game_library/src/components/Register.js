      ...
      .then((data) => {
        if (data.error) {
          ...
          setErrors((prevErrors) => ({
            ...prevErrors,
            error1: data.error.error1,
            error2: data.error.error2,
            error3: data.error.error3,
            error4: data.error.error4,
            error5: data.error.error5,
          }));
        } else {
          setCreated(true);
        }
  ...
  return (
    <Grid container alignContent="center" justifyContent={"center"}>
      <Grid item xs={3} id="login_block">
        ...
        <Grid item xs={12}>
          <Button id="register_submit" onClick={register}>
            Register
          </Button>
        </Grid>
      </Grid>
